...
public static class HaydenDbContextExtensions
{
	/// <summary>
	/// Sets up the required database engine and EF Core specific configuration.
	/// </summary>
	public static DbContextOptionsBuilder<HaydenDbContext> SetupHaydenDb(this DbContextOptionsBuilder<HaydenDbContext> builder, SourceConfig sourceConfig)
	{
		DatabaseType databaseType = sourceConfig.DbConnectionString.Contains("Data Source") ? DatabaseType.Sqlite : DatabaseType.MySql;

		return builder.SetupHaydenDb(databaseType, sourceConfig.DbConnectionString);
	}

	/// <summary>
	/// Sets up the required database engine and EF Core specific configuration.
	/// </summary>
	public static DbContextOptionsBuilder<HaydenDbContext> SetupHaydenDb(this DbContextOptionsBuilder<HaydenDbContext> builder, ConsumerConfig consumerConfig)
	{
		if (consumerConfig.DatabaseType == null || consumerConfig.DatabaseType == DatabaseType.None)
			throw new Exception("Invalid or unknown database type has been provided");

		return builder.SetupHaydenDb(consumerConfig.DatabaseType.Value, consumerConfig.ConnectionString);
	}

	/// <summary>
	/// Sets up the required database engine and EF Core specific configuration.
	/// </summary>
	public static DbContextOptionsBuilder<HaydenDbContext> SetupHaydenDb(this DbContextOptionsBuilder<HaydenDbContext> builder, DatabaseType databaseType, string connectionString)
	{
